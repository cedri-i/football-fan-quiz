...
const C=(name,abbr,league,power,color,style,query)=>({name,abbr,league,power,color,style,query,crest:`assets/crests-v2/${crestSlug(query)}.webp`});
const CLUBS=[
...
function generateOffers(){
 const current=game.club;const decision=clubWindowDecision(current);const total=rint(3,5);const slots=current?total-1:total;const recent=new Set(game.lastOffers||[]);
 ...
 const offers=[];if(current)offers.push(makeOffer(current,'留队',true));const moveType=decision.mode==='first'?'首份合同':decision.mode==='loan'?'租借':decision.mode==='release'?'自由签约':'转会';offers.push(...pool.map(club=>makeOffer(club,moveType,false)));game.lastOffers=offers.map(o=>o.club.name);$('#club-decision').textContent=firstLeague?`${game.nationality}球员从${firstLeague}起步`:decision.message;return offers
}
function clubWindowDecision(current){
 ...
 if(Math.random()>=Math.min(.94,exitRisk))return{mode:'stay',message:'俱乐部愿意提供留队合同'};if(game.age<=29&&gap>=4&&Math.random()<.62)return{mode:'loan',message:'俱乐部建议外租，但你仍可选择留队竞争'};if(game.age>=34)return{mode:'release',message:'续约谈判开放，也可以自由选择新球队'};return{mode:'sale',message:'收到转会报价，你仍然可以拒绝并留队'}
}
...
async function hydrateOfferBadges(){
 const slots=[...document.querySelectorAll('[data-crest]')];slots.forEach(slot=>{const club=offerList[+slot.dataset.crest]?.club;if(!club)return;const img=new Image();img.alt=`${club.name}队徽`;img.width=48;img.height=48;img.decoding='async';img.onload=()=>slot.replaceChildren(img);img.src=club.crest})
}
