...
async function json(url){for(let attempt=0;attempt<5;attempt++){const r=await fetch(url);if(r.ok)return r.json();if(r.status!==429)throw new Error(`${r.status}`);await sleep(2500*(attempt+1))}throw new Error('429')}
