...
const Q = [
 ...
 {id:'shirt',type:'球衣记忆',text:'一件旧球衣最珍贵的部分应该是什么？',a:[
  ['背后那个陪你长大的名字',['梅西','C罗','内马尔','阿扎尔','格列兹曼']],['胸前队徽，名字可以没有',['利物浦','曼联','拜仁','国米','米兰']],['一处冷门客场留下的污渍',['埃弗顿','诺丁汉森林','马赛','法兰克福']],['它来自国家队大赛的那个夏天',['阿根廷','法国','德国','西班牙','巴西']]]},
 ...
 {id:'rival',type:'宿敌反应',text:'德比前夜，哪句话最接近你？',a:[
  ['踢得好不好不重要，明天只能赢',['皇马','巴萨','马竞','曼联','利物浦']],['希望对面全员健康，这样赢了才算数',['阿森纳','曼城','拜仁','勒沃库森']],['已经把过去十年的名场面又看了一遍',['国米','米兰','尤文图斯','多特蒙德']],['嘴上说无所谓，赛程公布那天已经记住日期',['埃弗顿','热刺','罗马','拉齐奥']]]},
 ...
 {id:'club',type:'俱乐部性格',text:'你更愿意把十年交给怎样的俱乐部？',a:[
  ['冠军很多，要求也永远不会降低',['皇马','拜仁','曼城','尤文图斯']],['可能跌倒，但故事和歌不会断',['利物浦','曼联','米兰','马赛']],['先把一种踢法坚持成身份',['巴萨','阿贾克斯','阿森纳','西班牙']],['不需要所有人喜欢，只要自己人够坚定',['马竞','毕尔巴鄂竞技','多特蒙德','乌拉圭']]]},
...
function answer(tags,unknown=false){
 ...
 if(unknown)state.unknown++; else tags.forEach((t,i)=>state.scores[t]=(state.scores[t]||0)+(i<2?3:2));
 state.index++; render();
}
...
function finish(){
 const all=Object.entries(state.scores).sort((a,b)=>b[1]-a[1]).slice(0,5);
 const adjusted=all.map(([n,s],i)=>[n,Math.max(1,s*(1-.055*i))]); const sum=adjusted.reduce((a,x)=>a+x[1],0);
 let p=adjusted.map(([n,s])=>[n,Math.round(s/sum*100)]); p[0][1]+=100-p.reduce((a,x)=>a+x[1],0); state.result=p;
 ...
 const top=p[0][0]; $('#result-line').textContent=`主成分「${top}」，另有 ${p.length-1} 种看台反应被捕捉。`;
 $('#verdict').textContent=verdict(top,p); $('#result-no').textContent='NO. '+String(Math.floor(Math.random()*9999)).padStart(4,'0');
 show('result-screen');
}
function verdict(top,p){
 const second=p[1]?.[0]||'';
 const map={梅西:'你相信最好的足球先让人屏住呼吸，再让比分失去解释力。',C罗:'你对决定性时刻有近乎苛刻的迷恋，越大的舞台越值得下注。',巴萨:'你支持的不只是结果，还有一种球该如何移动的固执。',皇马:'你把压力视为舞台灯光：越亮，越应该有人站出来。',阿根廷:'你愿意陪一段漫长故事走到最后，足球对你从来不只九十分钟。',利物浦:'你相信看台能参与比赛，歌声与逆转之间并非巧合。'};
 return (map[top]||`你最强的信号落在「${top}」，但「${second}」让这份忠诚保留了意外。`) + (state.unknown>4?' 你跳过了不少陌生题，结果更偏向你明确表达过的部分。':'');
}
function resultText(){return '我的足球成分：'+state.result.map(x=>`${x[1]}% ${x[0]}`).join(' · ')+'｜看台成分局'}
